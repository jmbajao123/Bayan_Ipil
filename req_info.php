...
<head>
  ...
  <title>Certificate Request Information &mdash; Barangay Information System</title>
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Certificate Request Information</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="dashboard.php">Dashboard</a></div>
              <div class="breadcrumb-item">Certificate Request Information</div>
            </div>
          </div>
              ...
              <div class="col-12 col-lg-12 col-lg-12">
                <div class="card">
                  <div class="card-header">
                    <div class="col-lg-6">
                        <h2>Certificate Request Information</h2>
                    </div>
                  </div>
                  <div class="card-body">
...
// Get rc_id from URL
$rc_id = isset($_GET['rc_id']) ? intval($_GET['rc_id']) : 0;

// Initialize variables
$purpose = $c_name = $status = "No data found";

// Fetch purpose, cert_id, and status from req_cert and c_name from certificate in a single query using JOIN
$sql = "SELECT req_cert.purpose, req_cert.status, certificate.c_name 
        FROM req_cert 
        LEFT JOIN certificate ON req_cert.cert_id = certificate.cert_id 
        WHERE req_cert.rc_id = ?";

$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "i", $rc_id);
...
if ($row = mysqli_fetch_assoc($result)) {
    $purpose = $row['purpose'];
    $status = $row['status'];
    $c_name = $row['c_name'] ?? "Certificate not found"; // Handle NULL values
}
...
<form method="post" action="">
    <div class="col-lg-12">
        <div class="row">
            <div class="col-lg-4">
                <label><h4>Purpose</h4></label>
                <input type="text" name="purpose" class="form-control" value="<?php echo htmlspecialchars($purpose); ?>" readonly>
            </div>
            <div class="col-lg-4">
                <label><h4>Certificate Name</h4></label>
                <input type="text" name="c_name" class="form-control" value="<?php echo htmlspecialchars($c_name); ?>" readonly>
            </div>
            <div class="col-lg-4">
            ...
            <div class="col-lg-12">
                <a href="c_c.php" class="btn btn-outline-secondary">Back</a>
            </div>
        </div>
    </div>
</form>
