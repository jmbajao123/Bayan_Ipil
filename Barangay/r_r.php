...
<head>
  ...
  <title>Residence List Report &mdash; Barangay Information System</title>
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Residence List Report</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="dashboard.php">Dashboard</a></div>
              <div class="breadcrumb-item">Residence List Report</div>
            </div>
          </div>

          <div class="section-body">
            <h2 class="section-title">Residence List Report</h2>
            <div class="row">
                ...
                <div class="card">
									<div class="card-header">
									  ...
									  <div class="col-lg-12 d-flex justify-content-end">
									  	<a href="r_r_print.php" class="btn btn-outline-primary" target="_blank">Generate</a>
...
// Fetch data filtered by b_list_id and join with ps_list to get ps_name
            ...
            <tbody>
                <?php if ($result && mysqli_num_rows($result) > 0): 
                    while ($row = mysqli_fetch_assoc($result)): 
                        ...
                        $profile = !empty($row['profile']) && file_exists("uploads/" . $row['profile']) 
                            ? "uploads/" . $row['profile'] 
                            : "uploads/default.png"; // Use default image if none exists
                ?>
                        <tr>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $user_type = 'Residence Account'; // Added user type
    ...
    if (move_uploaded_file($_FILES['profile']['tmp_name'], $target_file)) {
        $query = "INSERT INTO residence (first_name, middle_name, last_name, suffix_name, date_birth, age, gender, civil_status, contact, profile, residence_id, ps_list_id, nationality, birthplace, email, password, con_pass, status, user_type, u_id, b_list_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "ssssssssssiisssssssii", $first_name, $middle_name, $last_name, $suffix_name, $date_birth, $age, $gender, $civil_status, $contact, $profile, $residence_id, $ps_list_id, $nationality, $birthplace, $email, $password, $con_pass, $status, $user_type, $u_id, $b_list_id);
