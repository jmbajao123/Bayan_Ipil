...
<head>
  ...
  <title>Approved Request List &mdash; Barangay Information System</title>
      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Approved Request List</h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
              <div class="breadcrumb-item">Approved Request List</div>
            </div>
          </div>

          <div class="section-body">
            <h2 class="section-title">Approved Request List</h2>
            <div class="row">
        ...
        <table class="table table-bordered table-md">
            ...
            <tbody>
                ...
                $query = "SELECT rc.r_id, rc.rc_id, rc.cert_id, rc.purpose, rc.status, 
                          ...
                          WHERE rc.b_list_id = '$b_list_id' AND rc.status = 'Approved'
                ...
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        ...
                        <tr>
                            ...
                            <td><?= $row['status']; ?></td>
                            <td>
    <?php if ($row['c_name'] === 'Barangay of Clearance'): ?>
        <a href="print_cert.php?rc_id=<?= urlencode($row['rc_id']); ?>" class="btn btn-outline-primary" target="_blank">Print</a>
    <?php elseif ($row['c_name'] === 'Certificate of Indigency'): ?>
        <a href="print_indegency.php?rc_id=<?= urlencode($row['rc_id']); ?>" class="btn btn-outline-primary" target="_blank">Print</a>
    <?php elseif ($row['c_name'] === 'Business Permit'): ?>
        <a href="print_busi_per.php?rc_id=<?= urlencode($row['rc_id']); ?>" class="btn btn-outline-primary" target="_blank">Print</a>
    <?php else: ?>
        <span class="text-muted">No print option</span>
    <?php endif; ?>
</td>
                        </tr>
                    <?php endwhile; ?>
