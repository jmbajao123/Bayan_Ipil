...
if (isset($_GET['cert_id'])) {
    $cert_id = intval($_GET['cert_id']); // Sanitize input

    // Fetch position details based on ID
    $query = "SELECT c_name, status FROM certificate WHERE cert_id = $cert_id";
    $result = mysqli_query($conn, $query);

    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $c_name = htmlspecialchars($row['c_name']); // Prevent XSS
        $status = htmlspecialchars($row['status']); // Store status
    }
}
?>

<form method="post" action="cert_info.php">
    <div class="col-lg-12">
        <div class="row">
            <div class="col-lg-6">
                <label>
            ...
            <div class="col-lg-12">
                <a href="cert.php" class="btn btn-outline-secondary">Back</a>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['cert_id'], $_POST['c_name'], $_POST['status'])) {
        $cert_id = intval($_POST['cert_id']); // Sanitize input
        $c_name = mysqli_real_escape_string($conn, $_POST['c_name']);
        $status = mysqli_real_escape_string($conn, $_POST['status']);

        // Update query
        $query = "UPDATE certificate SET c_name = '$c_name', status = '$status' WHERE cert_id = $cert_id";
        $result = mysqli_query($conn, $query);
