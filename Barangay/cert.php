...
<head>
  ...
  <title>Certificate List &mdash; Barangay Information System</title>
        ...
        <section class="section">
          <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
              ...
              <div class="breadcrumb-item">Certificate List</div>
            </div>
          </div>

          <div class="section-body">
            <h2 class="section-title">Certificate List</h2>
            <div class="row">
...
$total_query = "SELECT COUNT(*) AS total FROM certificate WHERE b_list_id = '$b_list_id'";
if ($status_filter) {
...
$query = "SELECT * FROM certificate WHERE b_list_id = '$b_list_id'";
if ($status_filter) {
...
<div class="card-header">
    <div class="col-lg-4">
        ...
        <select id="statusFilter" class="form-control" onchange="filterStatus()">
            ...
            <option value="Unavaialable" <?php echo ($status_filter == 'Unavaialable') ? 'selected' : ''; ?>>Unavaialable</option>
        </select>
    </div>
            ...
            <tbody>
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($row['cert_id']); ?></td>
                            <td><?php echo htmlspecialchars($row['c_name']); ?></td>
                            <td>
                                <div class="badge <?php echo ($row['status'] === 'Available') ? 'badge-success' : 'badge-danger'; ?>">
                                    <?php echo htmlspecialchars($row['status']); ?>
                                </div>
                            </td>
                            <td>
                                <a href="cert_info.php?cert_id=<?php echo urlencode($row['cert_id']); ?>" 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $check_ps_query = "SELECT * FROM certificate WHERE c_name = '$c_name' AND b_list_id = '$b_list_id'";
    ...
    $query = "INSERT INTO certificate (c_name, status, u_id, b_list_id, date) 
              VALUES ('$c_name', '$status', '$u_id', '$b_list_id', '$date')";
    $result = mysqli_query($conn, $query);
    ...
    if ($result) {
        ...
        echo "<script>
            ...
            }).then(() => { window.location.href='cert.php'; });
        </script>";
    } else {
        echo "<script>alert('Error inserting into certificate table');</script>";
    }
}
