...
// Retrieve the b_officials_id or another unique identifier from session or request
$b_officials_id = $_SESSION['b_officials_id']; // Adjust according to your session setup

// Database connection (use your own credentials and database info)
include 'conn.php';

// Fetch data from b_officials table based on b_officials_id
$query = "
    SELECT b.*, p.position_name
    FROM b_officials b
    LEFT JOIN b_positions p ON b.b_positions_id = p.b_positions_id
    WHERE b.b_officials_id = '$b_officials_id'
";
$result = mysqli_query($conn, $query);

// Check if a record is found
if ($row = mysqli_fetch_assoc($result)) {
    // Assign values from the database row to variables
    ...
    $contact = $row['contact'];
    $status = $row['status'];
    $profile = isset($row['profile']) ? "../Admin_Ipil/officials/" . $row['profile'] : "../Admin_Ipil/officials/default_image.jpg"; // Profile image path
    $position_name = $row['position_name']; // Fetch the position name from b_positions table
} else {
        ...
        <div class="row">
            ...
            <div class="col-lg-3">
                ...
                <select name="status" class="form-control" disabled>
                    <option value="Active Official" <?php echo ($status === 'Active Official') ? 'selected' : ''; ?>>Active Official</option>
                    <option value="Inactive Official" <?php echo ($status === 'Inactive Official') ? 'selected' : ''; ?>>Inactive Official</option>
                </select>
            </div>
